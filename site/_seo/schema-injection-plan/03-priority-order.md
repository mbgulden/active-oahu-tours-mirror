# Schema Injection Priority Order

This document details the rank-order injection schedule based on a traffic x schema gap calculation.

* **P0: Top-20 High-Value Pages (Highest organic ROI)**
* **P1: Mid-Traffic Core Pages (Pages 21-50)**
* **P2: Remaining English Pages (118 pages)**
* **P3: Japanese Locale Pages (All 89 JA pages)**

---


## P0: Top-20 High-Traffic Pages (Highest ROI)

These 20 pages drive over 75% of organic traffic and direct bookings for AOT. Injecting high-quality schema here offers immediate rich result opportunities.

| Rank | Relative Path | Page Type | Primary Schema Type |
| :--- | :--- | :--- | :--- |
| 1 | `index.html` | Homepage | `TravelAgency + LocalBusiness + Travel` |
| 2 | `chinamans-hat/index.html` | Other | `WebPage` |
| 3 | `kaneohe-sandbar/index.html` | Other | `WebPage` |
| 4 | `kayak-rentals/index.html` | Rental | `Product` |
| 5 | `rentals/index.html` | Location/Hub | `ItemList + TouristAttraction` |
| 6 | `activities/chinamans-hat-kayak-complete-self-guided-tour-guide/index.html` | Tour | `TouristTrip` |
| 7 | `activities/kaneohe-sandbar-kayak-ultimate-guide/index.html` | Tour | `TouristTrip` |
| 8 | `guides/kailua-beach-park/index.html` | Blog/Guide | `Article` |
| 9 | `guides/lanikai-beach/index.html` | Blog/Guide | `Article` |
| 10 | `sharks-cove-snorkeling/index.html` | Other | `WebPage` |
| 11 | `activities/sharks-cove-self-guided-snorkel/index.html` | Tour | `TouristTrip` |
| 12 | `kayak-kailua/index.html` | Other | `WebPage` |
| 13 | `guided-tours/index.html` | Location/Hub | `ItemList + TouristAttraction` |
| 14 | `self-guided/index.html` | Location/Hub | `ItemList + TouristAttraction` |
| 15 | `activities/chinamans-hat-self-guided-oahu-kayak-tour/index.html` | Tour | `TouristTrip` |
| 16 | `activities/kailua-bay-mokulua-island-self-guided-kayak-tour/index.html` | Tour | `TouristTrip` |
| 17 | `oahu-equipment-rentals/index.html` | Location/Hub | `ItemList + TouristAttraction` |
| 18 | `oahu-equipment-rentals/chinamans-hat-kayak-rentals/index.html` | Rental | `Product` |
| 19 | `multi-day-kayak-and-beach-gear-rentals/index.html` | Location/Hub | `ItemList + TouristAttraction` |
| 20 | `contact-us.html` | Contact | `ContactPage` |

---


## P1: Mid-Traffic Core Pages (Pages 21-50)

These pages cover secondary tours, popular equipment rentals, and high-impression blogs.

| Rank | Relative Path | Page Type | Primary Schema Type |
| :--- | :--- | :--- | :--- |
| 21 | `about-active-oahu/index.html` | Other | `WebPage` |
| 22 | `about-active-oahu-tours/index.html` | Other | `WebPage` |
| 23 | `faq/index.html` | FAQ | `FAQPage` |
| 24 | `faq/faq-chinamans-hat-kayak-hike/index.html` | FAQ | `FAQPage` |
| 25 | `faq/faq-oahu-beach-gear-rentals/index.html` | FAQ | `FAQPage` |
| 26 | `guides/index.html` | Location/Hub | `ItemList + TouristAttraction` |
| 27 | `guides/sea-turtles-oahu/index.html` | Blog/Guide | `Article` |
| 28 | `guides/ocean-kayaking-beginners-oahu/index.html` | Blog/Guide | `Article` |
| 29 | `activities/kailua-kayak-twin-islands-guided-tour/index.html` | Tour | `TouristTrip` |
| 30 | `activities/popoia-island-and-kailua-bay-guided-kayak-tour/index.html` | Tour | `TouristTrip` |
| 31 | `activities/kahana-rainforest-river-oahu-kayak-tour/index.html` | Tour | `TouristTrip` |
| 32 | `activities/kailua-e-bike-kau-kau-guided-adventure/index.html` | Tour | `TouristTrip` |
| 33 | `activities/lanikai-beach-self-guided-e-bike-snorkel-adventure/index.html` | Tour | `TouristTrip` |
| 34 | `rentals/oahu-beach-chair-rentals/index.html` | Rental | `Product` |
| 35 | `rentals/oahu-beach-umbrella-rentals/index.html` | Rental | `Product` |
| 36 | `rentals/oahu-stand-up-paddle-board-rentals-sup-hire/index.html` | Rental | `Product` |
| 37 | `rentals/oahu-tandem-kayak-rentals/index.html` | Rental | `Product` |
| 38 | `oahu-equipment-rentals/how-to-transport-kayaks-and-sups-from-our-shop-in-kailua-to-the-beach/index.html` | Rental | `Product` |
| 39 | `oahu-equipment-rentals/kayak-rental-delivery-locations/index.html` | Rental | `Product` |
| 40 | `oahu-kayaking-and-beach-adventures/index.html` | Location/Hub | `ItemList + TouristAttraction` |
| 41 | `oahu-kayaking-and-beach-adventures/ultimate-guide-for-kailua-beach-park-experience-windward-oahus-safest-and-most-adventurous-beach/index.html` | Blog/Guide | `Article` |
| 42 | `oahu-kayaking-and-beach-adventures/ultimate-guide-to-lanikai-beach/index.html` | Blog/Guide | `Article` |
| 43 | `oahu-kayaking-and-beach-adventures/chinamans-hat-kayak-adventure/index.html` | Blog/Guide | `Article` |
| 44 | `oahu-kayaking-and-beach-adventures/kaneohe-sandbar-self-guided-kayak-adventure/index.html` | Blog/Guide | `Article` |
| 45 | `oahu-kayaking-and-beach-adventures/mokulua-islands-self-guided-kayak-adventure/index.html` | Blog/Guide | `Article` |
| 46 | `oahu-kayaking-and-beach-adventures/popoia-island-kayaking-adventure/index.html` | Blog/Guide | `Article` |
| 47 | `tours/index.html` | Location/Hub | `ItemList + TouristAttraction` |
| 48 | `what-to-bring/index.html` | Other | `WebPage` |
| 49 | `why-choose-active-oahu/index.html` | Other | `WebPage` |
| 50 | `reviews/index.html` | Other | `WebPage` |

---


## P2: Remaining English Pages (118 pages)

These pages cover supporting blog posts, pagination pages, policy terms, and minor utilities.

<details>
<summary>Click to expand P2 page list</summary>

| Relative Path | Page Type | Primary Schema Type |
| :--- | :--- | :--- |
| `404.html` | Other | `WebPage` |
| `about-active-oahu-tours/awards/active-oahu-llc-wins-2022-tripadvisor-travelers-choice-award-for-tours-and-activities/index.html` | Other | `WebPage` |
| `about-active-oahu-tours/awards/active-oahu-recognized-one-top-10-hospitality-businesses-world/index.html` | Other | `WebPage` |
| `about-active-oahu-tours/awards/index.html` | Location/Hub | `ItemList + TouristAttraction` |
| `active-aloha-ambassador/index.html` | Other | `WebPage` |
| `active-oahu-llc-wins-2022-tripadvisor-travelers-choice-award-for-tours-and-activities/index.html` | Other | `WebPage` |
| `active-oahu-photo-gallery/index.html` | Other | `WebPage` |
| `activities.html` | Location/Hub | `ItemList + TouristAttraction` |
| `activities/aloha-aina-e-bike-adventure/index.html` | Tour | `TouristTrip` |
| `activities/chinamans-hat-kayak-rentals/index.html` | Rental | `Product` |
| `activities/chinamans-hat-oahu-kayak-tours/index.html` | Tour | `TouristTrip` |
| `activities/destination-yoga/index.html` | Tour | `TouristTrip` |
| `activities/east-oahu-self-guided-kayaking-experience/index.html` | Tour | `TouristTrip` |
| `activities/guided-mokulua-islands-kayak-tour-and-e-bike-adventure/index.html` | Tour | `TouristTrip` |
| `activities/haleiwa-paddleboarding/index.html` | Tour | `TouristTrip` |
| `activities/index.html` | Location/Hub | `ItemList + TouristAttraction` |
| `activities/kailua-flat-island-popoia-island-guided-kayak-e-bike-adventure/index.html` | Tour | `TouristTrip` |
| `activities/kaneohe-sandbar-kayak-rentals/index.html` | Rental | `Product` |
| `activities/kawela-bay-self-guided-kayak-tour/index.html` | Tour | `TouristTrip` |
| `activities/lanikai-beach-self-guided-snorkel/index.html` | Tour | `TouristTrip` |
| `activities/oahu-snorkel-tour/index.html` | Tour | `TouristTrip` |
| `activities/oahu-surf-lessons/index.html` | Tour | `TouristTrip` |
| `activities/page/2/index.html` | Location/Hub | `ItemList + TouristAttraction` |
| `activities/page/3/index.html` | Location/Hub | `ItemList + TouristAttraction` |
| `activities/rainforest-guided-hike/index.html` | Tour | `TouristTrip` |
| `activities/rainforest-oahu-kayak-tour.html` | Tour | `TouristTrip` |
| `activities/rainforest-oahu-stand-up-paddle-boarding/index.html` | Tour | `TouristTrip` |
| `activities/west-oahu-guided-snorkel-tour/index.html` | Tour | `TouristTrip` |
| `ariyoshis-summer-vacation-rainforest-kayak-tour-oahu-hawaii-2017/index.html` | Other | `WebPage` |
| `author/mbgulden/index.html` | Other | `WebPage` |
| `beach-gear-rentals/index.html` | Rental | `Product` |
| `become-a-partner/index.html` | Other | `WebPage` |
| `cancellation-policy/index.html` | Other | `WebPage` |
| `chinamans-hat-kayak-tour/index.html` | Tour | `TouristTrip` |
| `chinamans-hat-tide-guide/index.html` | Blog/Guide | `Article` |
| `contact-us/index.html` | Contact | `ContactPage` |
| `electric-bike-rentals/index.html` | Rental | `Product` |
| `faq-oahu-beach-gear-rentals/index.html` | FAQ | `FAQPage` |
| `guides/chinamans-hat-tide-guide/index.html` | Blog/Guide | `Article` |
| `guides/eating-your-way-windward-to-north-shore/index.html` | Blog/Guide | `Article` |
| `guides/electric-beach/index.html` | Blog/Guide | `Article` |
| `guides/oahu-kayak-safety-tide-guide/index.html` | Blog/Guide | `Article` |
| `guides/waimanalo-beach/index.html` | Blog/Guide | `Article` |
| `job-dashboard/index.html` | Other | `WebPage` |
| `job/hiring-kayak-delivery-driver-jobs-in-laie/index.html` | Other | `WebPage` |
| `join-the-team/index.html` | Other | `WebPage` |
| `kailua-ebike-route/index.html` | Other | `WebPage` |
| `kailua-kayak/index.html` | Other | `WebPage` |
| `kailua-oahu-storefront/index.html` | Other | `WebPage` |
| `kailua-town-history/index.html` | Other | `WebPage` |
| `kaneohe-bay-sandbar-kayak/index.html` | Tour | `TouristTrip` |
| `kaneohe-sandbar-tide-guide/index.html` | Blog/Guide | `Article` |
| `kayak-safety-guide/index.html` | Blog/Guide | `Article` |
| `kualoa-bay-guide/index.html` | Blog/Guide | `Article` |
| `laie-bay-goat-island-kayaking/index.html` | Other | `WebPage` |
| `lanikai-vs-hanauma-bay-snorkeling/index.html` | Blog/Guide | `Article` |
| `living-aloha-respectful-travel/index.html` | Other | `WebPage` |
| `mokolii/index.html` | Other | `WebPage` |
| `multi-activity-adventure-packages/index.html` | Other | `WebPage` |
| `multi-day-kayak-and-beach-gear-rentals/kayak-beach-gear-rental-partners/become-a-partner/index.html` | Rental | `Product` |
| `multi-day-kayak-and-beach-gear-rentals/kayak-beach-gear-rental-partners/index.html` | Rental | `Product` |
| `multi-day-rentals/index.html` | Location/Hub | `ItemList + TouristAttraction` |
| `oahu-equipment-rentals/extend-the-aloha-donate-your-beach-gear-before-you-fly/index.html` | Rental | `Product` |
| `oahu-equipment-rentals/kayak-rental-near-chinamans-hat/index.html` | Rental | `Product` |
| `oahu-equipment-rentals/page/2/index.html` | Location/Hub | `ItemList + TouristAttraction` |
| `oahu-equipment-rentals/standard-meeting-location-info/index.html` | Rental | `Product` |
| `oahu-hawaii-kayaking-guide/index.html` | Blog/Guide | `Article` |
| `oahu-hawaii-kayaking-guide/renting-a-kayak-and-paddling-to-mokolii-island-on-oahu/index.html` | Blog/Guide | `Article` |
| `oahu-kayak-safety-tide-index-map/index.html` | Other | `WebPage` |
| `oahu-kayaking-and-beach-adventures/best-places-to-kayak-on-oahu/index.html` | Blog/Guide | `Article` |
| `oahu-kayaking-and-beach-adventures/discover-oahus-best-snorkel-spot-at-electric-beach/index.html` | Blog/Guide | `Article` |
| `oahu-kayaking-and-beach-adventures/e-bike-rentals-in-kailua/index.html` | Blog/Guide | `Article` |
| `oahu-kayaking-and-beach-adventures/guide-to-towing-kayaks-with-e-bikes-in-kailua/index.html` | Blog/Guide | `Article` |
| `oahu-kayaking-and-beach-adventures/hidden-hawaiian-paradise-explore-kawela-bay-on-oahu/index.html` | Blog/Guide | `Article` |
| `oahu-kayaking-and-beach-adventures/honolulu-the-best-food-activities-and-social-scene-on-oahu/index.html` | Blog/Guide | `Article` |
| `oahu-kayaking-and-beach-adventures/kahana-river-kayak-adventure/index.html` | Blog/Guide | `Article` |
| `oahu-kayaking-and-beach-adventures/kailua-e-bike-kau-kau-guided-adventure/index.html` | Blog/Guide | `Article` |
| `oahu-kayaking-and-beach-adventures/kalama-beach-bodyboarding-adventure/index.html` | Blog/Guide | `Article` |
| `oahu-kayaking-and-beach-adventures/kaneohe-sandbar-kayak-experience/index.html` | Blog/Guide | `Article` |
| `oahu-kayaking-and-beach-adventures/kayak-deliveries-on-oahu/index.html` | Blog/Guide | `Article` |
| `oahu-kayaking-and-beach-adventures/lanikai-e-bike-and-snorkel-self-guided-tour/index.html` | Blog/Guide | `Article` |
| `oahu-kayaking-and-beach-adventures/lanikai-e-bike-snorkel-and-pillbox-hike-self-guided-adventure/index.html` | Blog/Guide | `Article` |
| `oahu-kayaking-and-beach-adventures/lanikai-pillbox-hike-adventure-guide/index.html` | Blog/Guide | `Article` |
| `oahu-kayaking-and-beach-adventures/rent-beach-gear-for-multiple-days-with-active-oahu/index.html` | Blog/Guide | `Article` |
| `oahu-kayaking-and-beach-adventures/store-your-bags-explore-kailua-bounce-luggage-storage-service-available-at-active-oahu/index.html` | Blog/Guide | `Article` |
| `oahu-kayaking-and-beach-adventures/the-ultimate-guide-for-the-north-shore-of-oahu/index.html` | Blog/Guide | `Article` |
| `oahu-kayaking-and-beach-adventures/the-ultimate-guide-to-exploring-windward-oahu/index.html` | Blog/Guide | `Article` |
| `oahu-kayaking-and-beach-adventures/top-5-things-to-do-on-oahu/index.html` | Blog/Guide | `Article` |
| `oahu-kayaking-and-beach-adventures/top-things-to-do-near-ko-olina-resort/index.html` | Blog/Guide | `Article` |
| `oahu-kayaking-and-beach-adventures/what-to-do-in-kailua-hidden-gems-and-must-sees/index.html` | Blog/Guide | `Article` |
| `oahu-launch-guide/index.html` | Blog/Guide | `Article` |
| `oahu-tour-packages/index.html` | Other | `WebPage` |
| `oahus-best-kayaking-trips/index.html` | Other | `WebPage` |
| `paa-answers/index.html` | FAQ | `FAQPage` |
| `privacy-policy/index.html` | Other | `WebPage` |
| `rentals/cruiser-oahu-beach-equipment-rental-package/index.html` | Rental | `Product` |
| `rentals/explorer-oahu-kayak-rental-package/index.html` | Rental | `Product` |
| `rentals/kailua-beach-bike-rentals/index.html` | Rental | `Product` |
| `rentals/kayak-sup-trolley/index.html` | Rental | `Product` |
| `rentals/oahu-beginner-surf-board-rentals/index.html` | Rental | `Product` |
| `rentals/oahu-boogie-board-rentals/index.html` | Rental | `Product` |
| `rentals/oahu-cooler-rentals/index.html` | Rental | `Product` |
| `rentals/oahu-dry-bag-rentals/index.html` | Rental | `Product` |
| `rentals/oahu-kayak-anchor-rentals/index.html` | Rental | `Product` |
| `rentals/oahu-life-vest-rentals/index.html` | Rental | `Product` |
| `rentals/oahu-snorkel-mask-and-fin-rentals/index.html` | Rental | `Product` |
| `rentals/oahu-tandem-kayak-rentals/kailua-kayak-rentals/index.html` | Rental | `Product` |
| `rentals/oahu-tandem-kayak-rentals/mokolii-kayak-rentals/index.html` | Rental | `Product` |
| `rentals/snorkel-gear-rentals/index.html` | Rental | `Product` |
| `reviews/page/2/index.html` | Other | `WebPage` |
| `reviews/page/3/index.html` | Other | `WebPage` |
| `reviews/page/4/index.html` | Other | `WebPage` |
| `reviews/page/5/index.html` | Other | `WebPage` |
| `sharks-cove-snorkeling-guide/index.html` | Blog/Guide | `Article` |
| `sharks-cove-vs-lanikai-snorkeling/index.html` | Blog/Guide | `Article` |
| `stand-up-paddleboard-rental/index.html` | Rental | `Product` |
| `trip-cancellation-insurance-terms-and-conditions.html` | Other | `WebPage` |
| `trip-cancellation-insurance-terms-and-conditions/index.html` | Other | `WebPage` |
</details>
## P3: Japanese Locale Pages (All 89 JA pages)

Japanese language versions of all core and supporting pages.

<details>
<summary>Click to expand P3 page list</summary>

| Relative Path | Page Type | Primary Schema Type |
| :--- | :--- | :--- |
| `ja/404.html` | Other | `WebPage` |
| `ja/about-active-oahu-tours/awards/active-oahu-llc-wins-2022-tripadvisor-travelers-choice-award-for-tours-and-activities/index.html` | Other | `WebPage` |
| `ja/about-active-oahu-tours/awards/active-oahu-recognized-one-top-10-hospitality-businesses-world/index.html` | Other | `WebPage` |
| `ja/about-active-oahu-tours/awards/index.html` | Location/Hub | `ItemList + TouristAttraction` |
| `ja/about-active-oahu-tours/index.html` | Other | `WebPage` |
| `ja/active-oahu-llc-wins-2022-tripadvisor-travelers-choice-award-for-tours-and-activities/index.html` | Other | `WebPage` |
| `ja/active-oahu-photo-gallery/index.html` | Other | `WebPage` |
| `ja/activities/aloha-aina-e-bike-adventure/index.html` | Tour | `TouristTrip` |
| `ja/activities/chinamans-hat-self-guided-oahu-kayak-tour/index.html` | Tour | `TouristTrip` |
| `ja/activities/east-oahu-self-guided-kayaking-experience/index.html` | Tour | `TouristTrip` |
| `ja/activities/guided-mokulua-islands-kayak-tour-and-e-bike-adventure/index.html` | Tour | `TouristTrip` |
| `ja/activities/haleiwa-paddleboarding/index.html` | Tour | `TouristTrip` |
| `ja/activities/index.html` | Location/Hub | `ItemList + TouristAttraction` |
| `ja/activities/kahana-rainforest-river-oahu-kayak-tour/index.html` | Tour | `TouristTrip` |
| `ja/activities/kailua-bay-mokulua-island-self-guided-kayak-tour/index.html` | Tour | `TouristTrip` |
| `ja/activities/kailua-e-bike-kau-kau-guided-adventure/index.html` | Tour | `TouristTrip` |
| `ja/activities/kailua-flat-island-popoia-island-guided-kayak-e-bike-adventure/index.html` | Tour | `TouristTrip` |
| `ja/activities/kailua-kayak-twin-islands-guided-tour/index.html` | Tour | `TouristTrip` |
| `ja/activities/lanikai-beach-self-guided-e-bike-snorkel-adventure/index.html` | Tour | `TouristTrip` |
| `ja/activities/oahu-surf-lessons/index.html` | Tour | `TouristTrip` |
| `ja/activities/page/2/index.html` | Location/Hub | `ItemList + TouristAttraction` |
| `ja/activities/page/3/index.html` | Location/Hub | `ItemList + TouristAttraction` |
| `ja/activities/popoia-island-and-kailua-bay-guided-kayak-tour/index.html` | Tour | `TouristTrip` |
| `ja/activities/rainforest-oahu-stand-up-paddle-boarding/index.html` | Tour | `TouristTrip` |
| `ja/activities/sharks-cove-self-guided-snorkel/index.html` | Tour | `TouristTrip` |
| `ja/activities/west-oahu-guided-snorkel-tour/index.html` | Tour | `TouristTrip` |
| `ja/ariyoshis-summer-vacation-rainforest-kayak-tour-oahu-hawaii-2017/index.html` | Other | `WebPage` |
| `ja/author/mbgulden/index.html` | Other | `WebPage` |
| `ja/become-a-partner/index.html` | Other | `WebPage` |
| `ja/cancellation-policy/index.html` | Other | `WebPage` |
| `ja/chinamans-hat-kayak-tour/index.html` | Other | `WebPage` |
| `ja/contact-us/index.html` | Contact | `ContactPage` |
| `ja/faq-oahu-beach-gear-rentals/index.html` | FAQ | `FAQPage` |
| `ja/faq/faq-chinamans-hat-kayak-hike/index.html` | FAQ | `FAQPage` |
| `ja/faq/faq-oahu-beach-gear-rentals/index.html` | FAQ | `FAQPage` |
| `ja/faq/index.html` | FAQ | `FAQPage` |
| `ja/guides/index.html` | Location/Hub | `ItemList + TouristAttraction` |
| `ja/index.html` | Homepage | `TravelAgency + LocalBusiness + Travel` |
| `ja/job-dashboard/index.html` | Other | `WebPage` |
| `ja/job/hiring-kayak-delivery-driver-jobs-in-laie/index.html` | Other | `WebPage` |
| `ja/join-the-team/index.html` | Other | `WebPage` |
| `ja/kailua-oahu-storefront/index.html` | Other | `WebPage` |
| `ja/kaneohe-bay-sandbar-kayak/index.html` | Other | `WebPage` |
| `ja/kayak-kailua/index.html` | Other | `WebPage` |
| `ja/multi-day-kayak-and-beach-gear-rentals/index.html` | Location/Hub | `ItemList + TouristAttraction` |
| `ja/multi-day-kayak-and-beach-gear-rentals/kayak-beach-gear-rental-partners/become-a-partner/index.html` | Rental | `Product` |
| `ja/multi-day-kayak-and-beach-gear-rentals/kayak-beach-gear-rental-partners/index.html` | Rental | `Product` |
| `ja/oahu-equipment-rentals/chinamans-hat-kayak-rentals/index.html` | Rental | `Product` |
| `ja/oahu-equipment-rentals/how-to-transport-kayaks-and-sups-from-our-shop-in-kailua-to-the-beach/index.html` | Rental | `Product` |
| `ja/oahu-equipment-rentals/index.html` | Location/Hub | `ItemList + TouristAttraction` |
| `ja/oahu-equipment-rentals/kayak-rental-delivery-locations/index.html` | Rental | `Product` |
| `ja/oahu-equipment-rentals/kayak-rental-near-chinamans-hat/index.html` | Rental | `Product` |
| `ja/oahu-equipment-rentals/page/2/index.html` | Location/Hub | `ItemList + TouristAttraction` |
| `ja/oahu-kayaking-and-beach-adventures/chinamans-hat-kayak-adventure/index.html` | Blog/Guide | `Article` |
| `ja/oahu-kayaking-and-beach-adventures/index.html` | Location/Hub | `ItemList + TouristAttraction` |
| `ja/oahu-kayaking-and-beach-adventures/kahana-river-kayak-adventure/index.html` | Blog/Guide | `Article` |
| `ja/oahu-kayaking-and-beach-adventures/kalama-beach-bodyboarding-adventure/index.html` | Blog/Guide | `Article` |
| `ja/oahu-kayaking-and-beach-adventures/kaneohe-sandbar-self-guided-kayak-adventure/index.html` | Blog/Guide | `Article` |
| `ja/oahu-kayaking-and-beach-adventures/lanikai-e-bike-and-snorkel-self-guided-tour/index.html` | Blog/Guide | `Article` |
| `ja/oahu-kayaking-and-beach-adventures/lanikai-e-bike-snorkel-and-pillbox-hike-self-guided-adventure/index.html` | Blog/Guide | `Article` |
| `ja/oahu-kayaking-and-beach-adventures/lanikai-pillbox-hike-adventure-guide/index.html` | Blog/Guide | `Article` |
| `ja/oahu-kayaking-and-beach-adventures/mokulua-islands-self-guided-kayak-adventure/index.html` | Blog/Guide | `Article` |
| `ja/oahu-kayaking-and-beach-adventures/popoia-island-kayaking-adventure/index.html` | Blog/Guide | `Article` |
| `ja/oahu-kayaking-and-beach-adventures/ultimate-guide-for-kailua-beach-park-experience-windward-oahus-safest-and-most-adventurous-beach/index.html` | Blog/Guide | `Article` |
| `ja/oahu-kayaking-and-beach-adventures/ultimate-guide-to-lanikai-beach/index.html` | Blog/Guide | `Article` |
| `ja/privacy-policy/index.html` | Other | `WebPage` |
| `ja/rentals/kailua-beach-bike-rentals/index.html` | Rental | `Product` |
| `ja/rentals/kayak-sup-trolley/index.html` | Rental | `Product` |
| `ja/rentals/oahu-beach-chair-rentals/index.html` | Rental | `Product` |
| `ja/rentals/oahu-beach-umbrella-rentals/index.html` | Rental | `Product` |
| `ja/rentals/oahu-beginner-surf-board-rentals/index.html` | Rental | `Product` |
| `ja/rentals/oahu-boogie-board-rentals/index.html` | Rental | `Product` |
| `ja/rentals/oahu-cooler-rentals/index.html` | Rental | `Product` |
| `ja/rentals/oahu-dry-bag-rentals/index.html` | Rental | `Product` |
| `ja/rentals/oahu-kayak-anchor-rentals/index.html` | Rental | `Product` |
| `ja/rentals/oahu-life-vest-rentals/index.html` | Rental | `Product` |
| `ja/rentals/oahu-snorkel-mask-and-fin-rentals/index.html` | Rental | `Product` |
| `ja/rentals/oahu-stand-up-paddle-board-rentals-sup-hire/index.html` | Rental | `Product` |
| `ja/rentals/oahu-tandem-kayak-rentals/index.html` | Rental | `Product` |
| `ja/rentals/oahu-tandem-kayak-rentals/kailua-kayak-rentals/index.html` | Rental | `Product` |
| `ja/rentals/oahu-tandem-kayak-rentals/mokolii-kayak-rentals/index.html` | Rental | `Product` |
| `ja/reviews/index.html` | Other | `WebPage` |
| `ja/reviews/page/2/index.html` | Other | `WebPage` |
| `ja/reviews/page/3/index.html` | Other | `WebPage` |
| `ja/reviews/page/4/index.html` | Other | `WebPage` |
| `ja/reviews/page/5/index.html` | Other | `WebPage` |
| `ja/sharks-cove-snorkeling/index.html` | Other | `WebPage` |
| `ja/stand-up-paddleboard-rental/index.html` | Other | `WebPage` |
| `ja/trip-cancellation-insurance-terms-and-conditions/index.html` | Other | `WebPage` |
</details>
